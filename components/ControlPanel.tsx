...
export const ControlPanel: React.FC<Props> = ({ date, setDate, birthYear, setBirthYear, onGenerate, viewMode }) => {
  ...
  // Helper to format Date to "YYYY-MM-DDTHH:mm" using local time values
    ...
    <div className={`transition-all duration-1000 ease-in-out flex flex-col items-center justify-center min-h-[60vh] z-20 ${layoutClass}`}>
        ...
        <div className="relative w-[300px] h-[300px] md:w-[400px] md:h-[400px]">
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[80%] h-[80%] bg-slate-950/80 backdrop-blur-md rounded-full border border-amber-500/30 shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col items-center justify-center p-6 z-10">
                ...
                <h2 className="text-2xl font-calligraphy text-amber-500 mb-6 tracking-widest glow-text">天圆地方</h2>

                <div className="w-full mb-4 relative group">
                    <label className="block text-[10px] text-slate-500 text-center mb-1 font-serif uppercase tracking-widest group-hover:text-amber-400 transition-colors">起局时间</label>
                    <input 
                        type="datetime-local" 
                ...
                <div className="w-full mb-8 relative group">
                    <label className="block text-[10px] text-slate-500 text-center mb-1 font-serif uppercase tracking-widest group-hover:text-amber-400 transition-colors">年命 (定位用神)</label>
                    <select 
                        value={birthYear}
                ...
                >
                    <span className="font-calligraphy text-xl text-amber-100 group-hover:text-amber-400">起</span>
                </button>

            </div>
        </div>
        
        <p className="mt-12 text-slate-500 font-serif text-xs tracking-[0.5em] opacity-60">设定时间与年命 · 演化奇门局</p>
    </div>
  );
};